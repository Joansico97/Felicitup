...
class UserResourceFirebase implements UserResource {
  ...
  Future<Either<ApiException, String>> login({required String email, required String password}) async {
    try {
      await _ref.read(authFirebaseProvider).signInWithEmailAndPassword(email: email, password: password);
      return const Right('');
    } on FirebaseAuthException catch (e) {
      return Left(ApiException(400, e.message!));
    } catch (e) {
  ...
  Future<Either<ApiException, String>> register({required String email, required String password}) async {
    try {
      final res =
          await _ref.read(authFirebaseProvider).createUserWithEmailAndPassword(email: email, password: password);
  ...
  @override
  Future<Either<ApiException, Map<String, dynamic>>> getUserData({required String idToken}) async {
